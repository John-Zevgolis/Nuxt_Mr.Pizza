...
const { data, pending } = await useAsyncData('synodeutika-data', async () => {
  const [titleData, itemsData] = await Promise.all([
    $fetch<{ title: string; url: string }>('/api/title', {
      query: { type: 'synodeutika' },
    }),
    $fetch<Item[]>('/api/items', {
      query: { type: 'synodeutika-items' },
    }),
  ]);
  ...
  return {
    ...
    url: titleData.url,
    synodeutika: itemsData,
  };
});

const synodeutika = computed(() => data.value?.synodeutika || []);
